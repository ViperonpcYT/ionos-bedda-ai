...
(function () {
  ...
  const ENDPOINT = (typeof onlyBikesApiUrl === 'function' ? onlyBikesApiUrl('/api/ai-engine.php') : '/api/ai-engine.php') + '?ajax=1';
  const LOG_ENDPOINT = typeof onlyBikesApiUrl === 'function' ? onlyBikesApiUrl('/api/log-event.php') : '/api/log-event.php';
  const SUPPORT_EMAIL = (window.ONLYBIKES_CONFIG && window.ONLYBIKES_CONFIG.supportEmail) || 'support@onlybikes.shop';
    ...
    root.innerHTML = `
      <style>
        ...
        .bedda-ai-btn {
          ...
          box-shadow: 0 10px 24px rgba(22, 163, 74, 0.35), 0 2px 6px rgba(0,0,0,0.12);
          cursor: pointer; display: flex; align-items: center; gap: 10px;
          font-weight: 600; font-size: 15px; transition: all 0.25s ease;
        }
        .bedda-ai-btn:hover { transform: translateY(-2px); box-shadow: 0 14px 32px rgba(22, 163, 74, 0.45); }
        .bedda-ai-btn svg { width: 22px; height: 22px; flex-shrink: 0; }
        .bedda-ai-btn .badge {
          background: white; color: #15803d; font-size: 10px; font-weight: 800;
        ...
        .bedda-ai-panel {
          ...
          background: #fafaf9; border-radius: 16px; overflow: hidden;
        ...
        .bedda-ai-msg.user {
          align-self: flex-end; background: #292524; color: white; border-bottom-right-radius: 4px;
        }
        .bedda-ai-msg.bot {
          align-self: flex-start; background: white; color: #292524;
          border: 1px solid #e7e5e4; border-bottom-left-radius: 4px;
          box-shadow: 0 1px 2px rgba(0,0,0,0.03);
        }
        .bedda-ai-msg.bot.typing { font-style: italic; color: #78716c; }
        .bedda-ai-disclaimer {
          font-size: 10px; color: #a8a29e; padding: 6px 14px 0 2px; align-self: flex-start; max-width: 85%;
          letter-spacing: 0.1px;
        }
        .bedda-ai-disclaimer a { color: #B45309; text-decoration: underline; }
        ...
        .bedda-ai-chip:hover { background: #dcfce7; border-color: #86efac; color: #166534; }
        .bedda-ai-input-wrap {
          border-top: 1px solid #e7e5e4; padding: 10px 12px; background: white; display: flex; gap: 8px;
        }
        .bedda-ai-input {
          flex: 1; border: 1px solid #d6d3d1; border-radius: 999px;
          padding: 10px 16px; font-size: 14px; outline: none; transition: border-color 0.15s;
          background: #fafaf9;
        }
        .bedda-ai-input:focus { border-color: #16a34a; background: white; }
        .bedda-ai-send {
          background: linear-gradient(135deg, #16a34a 0%, #15803d 100%);
        ...
        .bedda-ai-footnote {
          font-size: 10px; color: #a8a29e; text-align: center; padding: 6px 12px; background: white; border-top: 1px solid #f5f5f4;
        }
        .bedda-ai-footnote a { color: #15803d; text-decoration: none; }
      ...
      <button class="bedda-ai-btn" id="bedda-ai-toggle" aria-label="Open OnlyBikes assistant">
      ...
      <div class="bedda-ai-panel" id="bedda-ai-panel" role="dialog" aria-label="Bedda AI Assistant">
        <div class="bedda-ai-head">
          ...
          <div>
            <div class="title">OnlyBikes Assistant</div>
            <div class="subtitle">Fitment, parts, stock &amp; shipping — Canada e-moto</div>
          </div>
        ...
        <div class="bedda-ai-input-wrap">
          <input class="bedda-ai-input" id="bedda-ai-input" type="text" placeholder="Ask about Surron, Talaria, parts..." maxlength="500" autocomplete="off" />
        ...
        <div class="bedda-ai-footnote">
          AI-generated — verify fitment &amp; orders at <a href="mailto:${SUPPORT_EMAIL}">${SUPPORT_EMAIL}</a>.
        </div>
      </div>
    `;
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      ok = !!payload.success;
      reply = payload.text || "Sorry, I couldn't process that. Email " + SUPPORT_EMAIL + " and we'll help!";
    } catch (e) {
      reply = "Connection hiccup — try again or email " + SUPPORT_EMAIL + ".";
    }
  ...
  function refreshSuggestions() {
    ...
    const base = ['What fits a Surron Ultra Bee?', 'Do you ship across Canada?', 'What brake kits are in stock?'];
    if (path.includes('products')) {
      setSuggestions(['Ultra Bee brake kit in stock?', 'Supermoto wheels fit Talaria?', 'Titanium bolt kit specs?']);
    } else if (path.includes('bundles')) {
      setSuggestions(['What is in the Build Center kit?', 'Style Stack vs Trail kit?', 'Bundle shipping cost?']);
    } else if (path.includes('fitment')) {
      setSuggestions(['Sur-Ron vs Surron spelling?', 'E-Ride Pro compatibility?', 'How to verify fitment?']);
    } else if (path.includes('contact')) {
      setSuggestions(['How long does shipping take?', 'All sales final policy?', 'How do I track my order?']);
    } else {
      setSuggestions(base);
    }
  }
  ...
  function open() {
    ...
    if (!msgs.children.length) {
      appendMessage("Hi! I'm the OnlyBikes assistant. Ask about Surron, Talaria, and E-Ride parts, fitment, stock, or shipping.", 'bot');
      refreshSuggestions();
    }

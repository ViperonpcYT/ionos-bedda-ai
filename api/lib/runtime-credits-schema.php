...
if (!function_exists('runtime_credits_ensure_schema')) {
    function runtime_credits_ensure_schema(PDO $pdo): void
    {
        static $done = [];
        ...
        $tables = [
            ...
            ",
            'cloud_usage_daily' => "
                CREATE TABLE cloud_usage_daily (
        ...
        foreach ($tables as $name => $sql) {
            $exists = (bool) $pdo->query("SHOW TABLES LIKE '{$name}'")->fetchColumn();
            if (!$exists) {
                $pdo->exec($sql);
            }
        }

/**
 * Bedda Store - Client-Side Security Enhancements
...
class BeddaSecurity {
    ...
    setupSuspiciousActivityMonitoring() {
        ...
        document.addEventListener('keydown', (e) => {
            const now = Date.now();
            keystrokes.push({ key: e.key, time: now, interval: now - lastKeyTime });
            if (keystrokes.length > 50) keystrokes.shift();
            lastKeyTime = now;
        });
        
        // Check for bot-like behavior periodically
        setInterval(() => {
            this.checkBotBehavior(mouseMovements, keystrokes);
        }, 30000);
    }

    checkBotBehavior(mouseMovements, keystrokes) {
        // Very few mouse movements but many keystrokes = possible bot
        if (mouseMovements.length < 10 && keystrokes.length > 30) {
            this.logSuspiciousActivity('Possible bot: High keystrokes, low mouse movement');
        }
        
        // Very fast typing = possible bot
        if (keystrokes.length > 10) {
            const intervals = keystrokes.filter(k => k.interval > 0).map(k => k.interval);
            if (intervals.length > 0) {
                const avgInterval = intervals.reduce((a, b) => a + b, 0) / intervals.length;
                if (avgInterval < 30) { // Less than 30ms between keystrokes
                    this.logSuspiciousActivity('Possible bot: Very fast typing', { avgInterval });
                }
            }
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    beddaSecurity = new BeddaSecurity();
    
    // Make available globally for debugging
    window.beddaSecurity = beddaSecurity;
});

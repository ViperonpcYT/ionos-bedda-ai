class BeddaLogger {
    ...
    async logEvent(eventType, data) {
        ...
        // Page exit is high-priority: flush immediately with sendBeacon
        if (eventType === 'page_exit' || eventType === 'session_start') {
            this.flushBatch(true); // true = use sendBeacon for immediate flush
            return;
        }
        
        // Everything else goes into the batch queue
    ...
    async flushBatch(useBeacon = false) {
        ...
        try {
            ...
            this.offlineQueue.push(...eventsToSend);
            console.warn('Batch flush failed, queued for later:', error.message);
        } finally {
            this.isFlushing = false;
        }
    }
